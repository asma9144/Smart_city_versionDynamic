...
void ajouterBatiment()
{
    ...
    printf("B�timent ajoute avec succes ! ID: %d\n", b.id_batiment);
...
void Supprimer_Noeud(tCell **adr_batiment, int id)
{
    ...
    if (cour)
    {
        ...
        printf("Batiment supprimé avec succes !\n");
    }
    else
    {
...
void afficherBatimentsListe(tCell *adr_batiment) {
    ...
    while (cour) {
        compte++;
        printf("%d. ID: %d | Nom: %s | Adresse: %s, %s %d | Nb equipements: %d\n",
               compte,
...
void ajouterTypeEquipement() {
    ...
    // Saisie du nom
    printf("Nom du type (max 49 caracteres) : ");
    scanf(" %[^\n]", nom);
    strncpy(type.nom, nom, sizeof(type.nom) - 1);
    type.nom[sizeof(type.nom) - 1] = '\0';

    // Saisie de la description
    printf("Description (max 99 caracteres) : ");
    scanf(" %[^\n]", description);
    strncpy(type.description, description, sizeof(type.description) - 1);
    type.description[sizeof(type.description) - 1] = '\0';
...
void ajouterConsommation() {
    ...
    } while (rechercheConsommationParId(conso.id) != NULL);
    ...
    scanf("%f", &conso.consommation);

    // Date/heure de début (actuelle)
    conso.debut = time(NULL);

    // Date/heure de fin (pour l'instant égale à début)
